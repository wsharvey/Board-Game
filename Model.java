...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        if (place.row == 0) {
            Place belowPlace = pl(place.row + 1, place.col);
            action.accept(belowPlace);
            if (place.col == 0) {
                Place rightPlace = pl(place.row, place.col + 1);
                action.accept(rightPlace);
            } else if (place.col == _width - 1) {
                Place leftPlace = pl(place.row, place.col - 1);
                action.accept(leftPlace);
            } else {
                Place rightPlace = pl(place.row, place.col + 1);
                action.accept(rightPlace);
                Place leftPlace = pl(place.row, place.col - 1);
            ...
            if (place.row == _height - 1) {
                Place abovePlace = pl(place.row - 1, place.col);
                action.accept(abovePlace);
            } else {
                Place abovePlace = pl(place.row - 1, place.col);
                action.accept(abovePlace);
                Place belowPlace = pl(place.row + 1, place.col);
    ...
    boolean[] colorsPresent() {
        ...
        for (int columnNum = 0; columnNum < _height; columnNum++) {
            for (int rowNum = 0; rowNum < _width; rowNum++) {
                int value = _cells[columnNum][rowNum];
                if (!colors[value]) {
                    colors[value] = true;
                }
            }
    ...
    void setActiveRegionColor(int newColor) {
        for (Place element : _active) {
            int rowIndex = element.row;
            int columnIndex = element.col;
            _cells[rowIndex][columnIndex] = newColor;
        }
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> activePast = _active;
        HashSet<Place> adj = new HashSet<>();
        setActiveRegionColor(color);
        for (Place possibleAdj : _active) {
            if (!activePast.contains(possibleAdj)) {
                adj.add(possibleAdj);
            }
        } undo();
        return adj;
